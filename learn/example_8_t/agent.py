...
import random
viz = ClientFlowVisualizer(title=f"{AGENT_ID} Graph", port=random.randint(8710,8777))
...
async def validate(msg: Any) -> Optional[dict]:
    ...
    if content.get("to") != AGENT_ID: return
...
async def send_on_clock() -> str: 
    ...
    # s = await gpt_client.call({"model_id": "gpt-4o-mini", "prompt": "say hello in french"})
    # return {"message": s.response_json["output"][0]["content"][0]["text"], "to": "ChangeMe_Agent_7"}
    s = await claude_client.call({"model_id": "claude-3-haiku-20240307", "prompt": "say hello in french"})
    return {"message": s.response_json["content"][0]["text"], "to": "ChangeMe_Agent_8"}
