...
# Collect requirement files: agents\*\requirements.txt and learn\*\requirements.txt
$files = @()
...
# agent_InputAgent\requirements.txt
if (Test-Path -LiteralPath (Join-Path $PSScriptRoot "learn")) {
  $files += Get-ChildItem -LiteralPath (Join-Path $PSScriptRoot "learn") -Recurse -Filter "requirements.txt" -ErrorAction SilentlyContinue |
            Where-Object { -not $_.PSIsContainer }
}

if (-not $files -or $files.Count -eq 0) {
  Write-Host -ForegroundColor Yellow "No requirements.txt files found under agents\*\requirements.txt or learn\*\requirements.txt."
  return
}
